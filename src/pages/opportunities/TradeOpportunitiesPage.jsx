...
const TradeOpportunitiesPage = () => {
  ...
  return (
    <>
      <section className="py-8.5 px-2 bg-gradient-to-r from-indigo-900 via-blue-800 to-cyan-500 rounded-b-3xl overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {opportunities.map((opp, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="card group border-2 border-indigo-200/60 hover:border-indigo-400 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 hover:-translate-y-1"
                  style={{animation: `slideUp 0.6s ease-out ${idx * 0.1}s both`}}
                >
                  <div className="w-16 h-16 mb-4 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:shadow-indigo-500/50 transition-all duration-300">
                    <Icon className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="m-0 mb-2 text-lg font-black text-slate-900 group-hover:text-indigo-700 transition-colors">
                    {opp.title}
                  </h3>
